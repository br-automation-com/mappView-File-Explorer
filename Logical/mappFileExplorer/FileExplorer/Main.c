...
void CreateCompletePath(STRING path[DIR_PATH_SIZE+DAT_ITEM_SIZE], STRING target[DAT_ITEM_SIZE], UDINT return_path);
void InsertRecName(typMain_EXP *EXPL, STRING new_name[DIR_PATH_SIZE], STRING size[DAT_SIZE_SIZE], UINT SIDE);
void SelectItem(typMain_EXP *EXPL, USINT SOURCE);
BOOL IsInstr(STRING string1[DAT_ITEM_SIZE], STRING string2[DAT_ITEM_SIZE]);
		...
		switch (step)
		{
			...
			case STP_WAIT:
				...
				if (EXPL.CMD[SIDE].dir_create == 1) 	
				{
					...
					else if(brsstrlen((UDINT)EXPL.PAR[SIDE].dir_name) + 5 >= DAT_ITEM_SIZE) CreateError(50201, SIDE, "directory name exceeds maximum length", &EXPL);
				...
				if (EXPL.CMD[SIDE].delete == 1) 	
				{
					...
					if (brsstrcmp((UDINT)EXPL.DAT[SIDE].item_list[EXPL.VIS[SIDE].item_selected], (UDINT)"") == 0)	CreateError(50000, SIDE, "item name is empty", &EXPL);
						/* Check if new directory name fits into string											*/
					else if(brsstrlen((UDINT)EXPL.DAT[SIDE].item_list[EXPL.VIS[SIDE].item_selected]) + brsstrlen((UDINT)EXPL.PAR[SIDE].dir_path) + 1 >= DIR_PATH_SIZE + DAT_ITEM_SIZE) CreateError(50202, SIDE, "directory path exceeds maximum length", &EXPL);
						/* Make sure that user chooses a valid item												*/
			...
			case STP_READ_DIR:
				...
				brsmemset((UDINT)&EXPL.VIS[SIDE], 0, sizeof(EXPL.VIS[SIDE]));
			...
			case STP_READ_DIR_2:
				...
				if (DRead.status == OK)
				{
					...
					if (EXPL.DAT[SIDE].item_num < sizeof(EXPL.DAT[SIDE].item_list)/sizeof(EXPL.DAT[SIDE].item_list[0]))
					{
						...
						if ((brsstrlen((UDINT)EXPL.PAR[SIDE].item_filter) == 0) || (IsInstr((UDINT)lDirReadData.Filename, EXPL.PAR[SIDE].item_filter)))
						{
							/* ---------------------------------------------------------------------------- */		
							/* Read file information														*/
							if (lDirReadData.Mode == fiFILE)
							{					
								if (brsstrlen((UDINT)lDirReadData.Filename) < DAT_ITEM_SIZE-5)
								{
									...
									brsmemcpy((UDINT)tmp_str1 + DAT_ITEM_SIZE - brsstrlen((UDINT)tmp_str2) - 1, (UDINT)tmp_str2, brsstrlen((UDINT)tmp_str2));
			...
			case STP_FILE_COPY:
				...
				if ((EXPL.VIS[SIDE].item_selected < sizeof(EXPL.DAT[SIDE].item_list)/sizeof(EXPL.DAT[SIDE].item_list[0])) && (EXPL.VIS[SIDE].item_selected < EXPL.DAT[SIDE].item_num))
				{
					...
					{		
						...
						if(brsstrlen((UDINT)EXPL.DAT[SIDE].item_list[EXPL.VIS[SIDE].item_selected]) < DAT_ITEM_SIZE)
						{								
							/* ---------------------------------------------------------------------------- */		
...
void CreateCompletePath(STRING path[DIR_PATH_SIZE+DAT_ITEM_SIZE], STRING target[DAT_ITEM_SIZE], UDINT return_path)
{
	STRING 	tmp[DAT_ITEM_SIZE];
	brsmemset((UDINT)&tmp, 0, DAT_ITEM_SIZE);
...
/********************************************************************************************************/
void InsertRecName(typMain_EXP *EXPL, STRING new_name[DAT_ITEM_SIZE], STRING size[DAT_SIZE_SIZE], UINT SIDE)
{
	DINT 				lx		= 0;
	DINT 				ly		= 0;
	DINT  				lSort	= 0;
	...
	brsmemmove((UDINT)EXPL->DAT[SIDE].item_list[lx+1],(UDINT)EXPL->DAT[SIDE].item_list[lx], (EXPL->DAT[SIDE].item_num - lx) * (DAT_ITEM_SIZE + 1));
	brsstrcpy((UDINT)EXPL->DAT[SIDE].item_list[lx], (UDINT)new_name);

	/* Create data structure for vis size listbox															*/
	brsmemmove((UDINT)EXPL->DAT[SIDE].item_size[lx+1],(UDINT)EXPL->DAT[SIDE].item_size[lx], (EXPL->DAT[SIDE].item_num - lx) * (DAT_SIZE_SIZE + 1));
